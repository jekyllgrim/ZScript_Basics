...
🔵 [<< Previous: Event Handlers](11_Event_Handlers.md) 🔵 [>> Next: Weapons, PSprite and overlays
...
- Every PlayerPawn instance has a `player` pointer to the PlayerInfo struct that controls it. Example: if you have a `ppawn` pointer to a player pawn, `ppawn.player` will be the pointer to the relative PlayerInfo struct.

- Conversely, every PlayerInfo struct has a `mo` pointer to its PlayerInfo ("mo" stands for "map object"). Example: if you have a `pplayer` pointer to a PlayerInfo struct, `pplayer.mo` will be a pointer to the relative PlayerPawn.
...
- PlayerInfo has a `mo` pointer to its PlayerInfo
...
Since it's a number, using `players[consoleplayer]` you can access the PlayerInfo struct of the owner of the game. Do note, that **you should be VERY careful with this pointer**. Doom multiplayer is wholly reliant on synchronization: it only works as long as all the gameplay data is synced and identical between all the players. However, if you perform any code that is somehow related to `consoleplayer`, that code will not be synced across the network, and if that code has any effect on the playsim whatsoever, it'll immediately break.
