...
The word "Inventory" within GZDoom implies two things:
...
* `Inventory` is a base GZDoom class that all items are based on. Items are actors that can be picked up and are normally placed in the toucher's inventory.
...
It's also worth noting that when an item is picked up, the actual item object that was placed on the map *doesn't move* anywhere; instead it creates a copy of itself in the player's inventory, and then the original destroys itself.

### The process

This process of obtaining an item involves several steps that are handled within the item's `TryPickup()` function (covered further in the chapter):

1. First the item will check if the player already has a maximum amount of this item class available. If they do, nothing happens and the item stays in the world.

2. Then the item lets itself be processed by other objects, such as other items with a `HandlePickup()` override (more on that later).

3. If the player has *never* picked this item before in the current play session, the item does one of two things, depending on the game mode:
   
   **In singleplayer**: the item moves itself into the player pawns' inventory, setting the player pawn as its `owner`. The item gains a `bNoSector` flag so it's no longer rendered or attached to any sectors.
   
   **In multiplayer**: the item creates an instance of itself and places it in the player's inventory. Then it visually disappears and sets itself up to be respawned.

4. If the player *has* picked up this item before, the item doesn't place itself into their inventory; instead it gets a pointer to the instance of itself that is already in player's inventory and increases its `amount`, then the pickup destroys itself (or sets up itself to be respawned in multiplayer). This is why, when picking up multiple items of the same type, you don't get multiple item instances in your inventory; instead you get one instance of that item with a specific `amount` value.
...
Here the main `Inventory` properties:
...
- `Inventory.pickupmessage` — a string printed on the screen when you pick up the item.

- `Inventory.pickupsound` — the sound played when the item is picked up. As with all sounds, this takes an [SNDINFO](https://zdoom.org/wiki/SNDINFO) definition, not a file name.
...
* `INVENTORY.ALWAYSPICKUP` — the item is always picked up, even if the player already has maximum amount of it (the pickupmessage and pickupsound are used, but the item is destroyed afterwars).
...
* `INVENTORY.PERSISTENDPOWER` — the item is kept when travelling between levels (normally items are stripped, unless the levels belong to the same [hub](https://zdoom.org/wiki/Hub).
...
This function is called every tic while the item has a valid `owner`. It's similar to `Tick()`, except `Tick()` is run always, even if the item has no `owner`. This function can be used to add behavior to the item, such as modifying something on the owner. It's a near function if you want to inject some special behavior into the actor without replacing the actor. 

`Inventory`'s own `DoEffect()` is empty, but it's recommended to call `super.DoEffect()` when overriding it, for consistency.

Normally `DoEffect()` shouldn't be run if the `owner` is null, but under very special circumstances it can happen. One notable example is when the owner has a `Death` state sequence with a total duration of 0: if that happens, when the owner gets killed, the item's `DoEffect()` will still run during that game's tic before the item is destroyed. 

> *Note:* 0-tic `Death` sequences are a very bad practice and should never be used; at least one empty state (such as `TNT1 A 1`) should always be present. Still, since edge cases like this are possible, it's important to null-check the `owner`.
...
There are also some subclasses of `Inventory` that are defined to be useable: for example, `HealthPickup` (a base for useable healing items, such as Hexen's Quartz Flask) or `PowerupGiver` (a pickup for powerups that gives a related `PowerUp` item upon use). Note that all Doom's PowerupGivers have the `AUTOACTIVATE` flag set, so they're used automatically on pickup, but for example in Hexen PowerupGivers are usually useable manually.
...
Checking for `passive` you can decide whether you want to modify outgoing damage (`passive` is false) or incoming damage (`passive` is true).

This function is documented and has examples [on ZDoom Wiki](https://zdoom.org/wiki/ModifyDamage).

This function's passive part is not called if the damage inflictor is set up to ignore protection, such as using a [`DamageMobj()`](https://zdoom.org/wiki/DamageMobj) call with `DMG_NO_PROTECT` flag. Its active part is, similarly, not called if the damage inflictor used by the player 
...
Armor pickups are never placed in your inventory; instead they get access to `BasicArmor` in the toucher's inventory and mofiy its properties, the get removed (or set up for respawning). This follows the same logic as health pickups that modify the toucher's health and are then removed. 
...
PowerupGivers are also placed into player's inventory by default, and they give the `owner` their related powerup in their `Use()` function. Most powerup givers in Hexen, for example, are meant to be used from the inventory bar, whereas in Doom all powerup givers have +INVENTORY.AUTOEFFECT set, which makes them activate immediately upon pickup.
